...
;(function (Ps) {
  ...
  window[Ps] = function (img, width, height) {
    ...
    if (this instanceof window[Ps]) {
      ...
      if (!isNaN(img)) {
        ...
        var imgWidth = parseInt(computedStyle.getPropertyValue('width'))
        var imgHeight = parseInt(computedStyle.getPropertyValue('height'))
  ...
  window[Ps].prototype = {
    ...
    show: function (selector, isFast, flag) {
      ...
      if (this.layers.length == 0) {
        ...
        for (var i = 0; i < this.layers.length; i++) {
          ...
          if (layers[layers.length - 1] && layers[layers.length - 1][0].type == 1) {
            currLayer = layers[layers.length - 1][0]
          }
    ...
    drawRect: function (selector) {
      ...
      for (var m = 0; m < 255; m++) {
        if (result[m] > max) max = result[m]
      }

      for (var j = 0; j < 255; j++) {
        var currY = result[j] || 0
        currY = canvas.height - currY / max * 0.8 * canvas.height
        context.lineTo(j / 256 * canvas.width, currY, 1, 1)
      }
    ...
    rotate: function (deg) {
      ...
      var matrix = [Math.cos(r), Math.sin(r), -Math.sin(r), Math.cos(r), 0, 0]
...
;(function (Ps) {
  window[Ps].module('dorsyMath', function (P) {
    var M = {
      ...
      applyMatrix: function (imgData, matrixArr, low) {
      ...
      applyInHSI: function (imgData, func) {
        // 在hsi空间上应用func
        /*
                 * function(i){
                 *      i.H += 3;
      ...
      applyInCoordinate: function (imgData, func) {
        // 在坐标空间上应用func
        /*
                 * function(dot){
                 *
          ...
          if (actionMethod == 'act') {
            ...
            function checkReadyState () {
              ...
              if (action[1].readyState) {
                // 构造参数
                var params = [aiObj.imgData, action[1].imgData].concat(action.slice(2))
      ...
      getFun: function (fun) {
        var Effects = {
          ...
          sketch: function () {
            // 素描
            var _this = this.clone()
            return this.add(_this.act('反色').act('高斯模糊', 8), '颜色减淡')
              .act('toGray')
              .act('锐化', 1)
          },
          ...
          vintage: function () {
            ...
            return this.act('灰度处理').add(
              window[Ps](this.canvas.width, this.canvas.height, '#808080')
                .act('添加杂色')
                .act('高斯模糊', 4)
          ...
          rough: function () {
            return this.add(
              window[Ps](this.canvas.width, this.canvas.height, '#000')
                .act('喷点')
                .act('反色')
                .act('浮雕效果'),
              '叠加'
    ...
    var M = {
      process: function (imgData, arg) {
        ...
        var f = function (x, p0, p1, p2, p3) {
          // 基于三次贝塞尔曲线
          return (
            p0 * Math.pow(1 - x, 3) + 3 * p1 * x * Math.pow(1 - x, 2) + 3 * p2 * x * x * (1 - x) + p3 * Math.pow(x, 3)
          )
        }
